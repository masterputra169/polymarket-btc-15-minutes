...
- **Pipeline-v2 candidate (2026-09-24, NOT deployed)**: `backtest/ml_training/candidates/20260924_pipeline_v2/` — 14,607 markets 2026-03-28 → 09-23; test acc 76.9%, AUC 0.854, ECE 0.022 (XGB) / 0.034 (ens), weights 0.75/0.25; Brier skill vs same-instant market **+7.0%** (test n=2,176), but **−2.1%** vs a look-ahead-interpolated market on 3,728 unseen rows — the lookup prints ~1/min, so the truth for fresh live prices lies between: market-level, not a proven edge. Passes every gate (rel checks skipped v1→v2). Verified through the live v2 path on real klines. Deploy was blocked by the auto-mode permission classifier (production deploy) and left to the user: back up public/ml → `public/ml/backups/<tag>_*` + `rollback_*`, copy the three JSONs in, commit, `railway up --service bot --ci`. Deploying it restarts the dry-run evaluation window (different trade selection).
