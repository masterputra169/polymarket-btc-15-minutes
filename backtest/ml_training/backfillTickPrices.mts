...
type LookupEntry = Record<string, any> & { prices?: unknown[] };
...
const CONCURRENCY = parseInt(String(ARGS.concurrency ?? '5'), 10);
const LIMIT = parseInt(String(ARGS.limit ?? '0'), 10);
const CHECKPOINT_N = parseInt(String(ARGS.checkpoint ?? '500'), 10);
...
async function main() {
  ...
  const lookup = JSON.parse(fs.readFileSync(LOOKUP_PATH, 'utf-8')) as Record<string, LookupEntry>;
