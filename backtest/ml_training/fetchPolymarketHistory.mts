...
const MAX_EVENTS = ARGS.limit ? parseInt(ARGS.limit, 10) : 50000;
...
async function fetchWithRetry<T = unknown>(url, retries = 3): Promise<T | undefined> {
...
async function fetchAllResolvedMarkets() {
  ...
  while (allMarkets.length < MAX_EVENTS) {
    try {
      ...
      const events = await fetchWithRetry<any[]>(url);
