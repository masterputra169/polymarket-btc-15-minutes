...
def traverse_lgb_tree(node: dict, features: np.ndarray) -> float:
    ...
    if np.isnan(val):
        return traverse_lgb_tree(node['left_child'] if default_left else node['right_child'], features)
    if val <= thr:
...
def verify_browser_inference(model: Any, tree_info: list[dict], init_score: float,
                             X: np.ndarray, n_rows: int = 5) -> float:
...
def build_lgb_browser_model(tree_info: list[dict],
                            *,
                            ...
                            cv_auc: float,
                            cv_acc: float,
                            ensemble_weights: dict[str, float]) -> dict:
