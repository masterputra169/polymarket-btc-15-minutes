...
cutoff = __import__("datetime").datetime.now(__import__("datetime").timezone.utc) - __import__(
    "datetime"
).timedelta(days=DAYS)
...
stale_sec = int(datetime.now(UTC).timestamp()) - latest_ts if latest_ts else None
if not markets and latest_ts and stale_sec is not None and stale_sec > 3 * 86400:
    latest_iso = datetime.fromtimestamp(latest_ts, UTC).isoformat()
    print(
...
if new:
    ...
    print("  These rows will NOT contribute to feature generation in")
    print("  generateTrainingData.mts (Polymarket features 44-47 need price history).")
