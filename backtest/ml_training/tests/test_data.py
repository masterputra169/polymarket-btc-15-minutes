...
class TestLoadTrainingData:
    def test_metadata_column_never_becomes_a_feature(self, tmp_path) -> None:
        data = load_training_data(_write(tmp_path, _frame(10, slugs=list(range(10)))),
                                  log=lambda _: None)
    ...
    def test_zero_features_blanks_the_column_without_dropping_it(self, tmp_path) -> None:
        lines: list[str] = []
        data = load_training_data(_write(tmp_path, _frame(10)),
                                  zero_features=['feat_a'], log=lines.append)
    ...
    def test_unknown_zero_feature_warns_instead_of_failing(self, tmp_path) -> None:
        lines: list[str] = []
        load_training_data(_write(tmp_path, _frame(10)),
                           zero_features=['nope'], log=lines.append)
...
class TestTemporalSplit:
    ...
    def test_embargo_gap_at_the_test_boundary(self) -> None:
        X, y = self._xy()
        embargo = 16
        s = temporal_split(X, y, test_size=0.15, holdout_frac=0.0,
                           embargo=embargo, log=lambda _: None)
    ...
    def test_embargo_gap_at_the_holdout_boundary(self) -> None:
        X, y = self._xy()
        embargo = 16
        s = temporal_split(X, y, test_size=0.15, holdout_frac=0.125,
                           embargo=embargo, log=lambda _: None)
    ...
    def test_features_and_labels_stay_row_aligned(self) -> None:
        ...
        s = temporal_split(X, y, test_size=0.15, holdout_frac=0.125,
                           embargo=16, log=lambda _: None)
        for Xs, ys in ((s.X_train, s.y_train), (s.X_test, s.y_test),
                       (s.X_holdout, s.y_holdout), (s.X_train_full, s.y_train_full)):
