...
def _metrics(**overrides) -> XgbEvalMetrics:
    base = dict(
        accuracy=0.7712345, auc=0.8643219, f1=0.7512345, logloss=0.4912345,
        brier=0.1612345,
        calibration={'ece': 0.0451234, 'mce': 0.1234567, 'bins': [{'count': 3}]},
        high_conf_accuracy=0.912345, high_conf_ratio=31.4159, high_conf_count=421,
        high_conf_threshold=0.68,
        cv_auc=0.8412345, cv_acc=0.7512345,
        cv_test_acc_gap=0.02, cv_test_auc_gap=0.0231,
        holdout_accuracy=0.7412345, holdout_auc=0.8312345,
        test_holdout_acc_gap=0.03, test_holdout_auc_gap=0.0331,
        test_samples=1234, holdout_samples=567,
...
def _validation() -> ValidationInfo:
    return ValidationInfo(test_size=0.15, holdout_frac=0.125, strict_holdout=True,
                          threshold_source='oof_cv', calibration_eval_source='holdout',
                          test_samples=1234, holdout_samples=567)


def _browser_model(**overrides) -> dict:
    kwargs = dict(
        feature_cols=FEATURE_COLS, feature_cols_orig=BASE_COLS,
        engineered_features=ENGINEERED, best_iteration=41, optimal_threshold=0.68,
        platt_a=1.04, platt_b=0.02, platt_on_logits=True,
        pruned_features=['macd_hist'], pre_excluded_features=['funding_rate_change'],
        zero_features=[], recency_enabled=False, recency_halflife=90,
        signal_modifiers={'rsi': 1.2}, phase_thresholds=None,
        params={'objective': 'binary:logistic', 'max_depth': 5},
        use_optuna=True, validation=_validation(), metrics=_metrics(),
    )
...
class TestSignalModifiers:
    ...
    def test_mean_is_normalised_to_about_one(self) -> None:
        ...
        importance = {feat: 12.0 / len(feats)
                      for feats in SIGNAL_FEATURE_MAP.values() for feat in feats}
...
class TestMetricsBlock:
    def test_deploy_gate_field_names_are_all_present(self) -> None:
        ...
        for name in ('accuracy', 'auc', 'brier', 'calibration_ece', 'calibration_mce',
                     'high_conf_accuracy', 'high_conf_ratio', 'cv_test_acc_gap',
                     'holdout_accuracy', 'test_holdout_acc_gap'):
    ...
    def test_key_order_is_stable(self) -> None:
        ...
        assert list(build_metrics_block(_metrics())) == [
            'accuracy', 'auc', 'f1', 'logloss', 'brier',
            'calibration_ece', 'calibration_mce',
            'high_conf_accuracy', 'high_conf_ratio', 'high_conf_count',
            'high_conf_threshold', 'cv_auc', 'cv_acc',
            'cv_test_acc_gap', 'cv_test_auc_gap',
            'holdout_accuracy', 'holdout_auc',
            'test_holdout_acc_gap', 'test_holdout_auc_gap',
            'test_samples', 'holdout_samples',
            'confidence_buckets', 'calibration_bins',
        ]
    ...
    def test_absent_holdout_exports_null_not_zero(self) -> None:
        block = build_metrics_block(_metrics(holdout_accuracy=None, holdout_auc=None,
...
class TestBrowserModel:
    def test_top_level_key_order_is_stable(self) -> None:
        assert list(_browser_model()) == [
            'format', 'version', 'num_features', 'num_trees', 'feature_names',
            'original_features', 'engineered_features', 'best_iteration',
            'optimal_threshold', 'platt_a', 'platt_b', 'platt_on_logits',
            'pruned_features', 'pre_excluded_features', 'zero_features',
            'recency_weighting', 'signal_modifiers', 'phase_thresholds',
            'params', 'training_method', 'validation', 'metrics', 'trees',
        ]
...
class TestNormExport:
    def _norm(self, **overrides) -> dict:
        kwargs = dict(
            feature_cols=FEATURE_COLS, feature_cols_orig=BASE_COLS,
            platt_a=1.04, platt_b=0.02, platt_on_logits=True,
            pruned_features=[], signal_modifiers={'rsi': 1.2},
            phase_thresholds=None, holdout_frac=0.125, holdout_start_idx=900,
        )
        kwargs.update(overrides)
    ...
    def test_means_and_stds_come_from_the_supplied_block(self) -> None:
        X = np.vstack([np.zeros((10, len(FEATURE_COLS))),
                       np.full((10, len(FEATURE_COLS)), 4.0)]).astype(np.float32)
        norm = self._norm(X=X)
    ...
    def test_key_order_is_stable(self) -> None:
        assert list(self._norm()) == [
            'version', 'means', 'stds', 'feature_names', 'num_features',
            'original_features', 'platt_a', 'platt_b', 'platt_on_logits',
            'pruned_features', 'engineered_feature_specs', 'signal_modifiers',
            'phase_thresholds', 'train_samples', 'holdout_frac', 'holdout_start_idx',
        ]
    ...
    def test_every_engineered_feature_has_a_written_down_spec(self) -> None:
        ...
        base = [
            'delta_1m_pct', 'delta_3m_pct', 'rsi_norm', 'rsi_slope', 'vwap_dist',
            'vwap_slope', 'macd_line', 'macd_hist', 'vol_ratio_norm',
            'multi_tf_agreement', 'bb_percent_b', 'bb_squeeze', 'atr_pct_norm',
            'vol_delta_buy_ratio', 'ema_cross_signal', 'ema_dist_norm',
            'stoch_k_norm', 'ha_signed_consec', 'regime_trending',
            'regime_confidence', 'regime_mean_reverting', 'ha_is_green',
            'market_price_momentum', 'orderbook_imbalance',
            'crowd_model_divergence', 'rule_confidence',
        ]
...
class TestTrainingReport:
    def _report(self, **overrides) -> list[str]:
        kwargs = dict(
            use_optuna=True, tune_trials=150, winner='Optuna_trial_7', threshold=0.68,
            n_trees=42, feature_cols=FEATURE_COLS, feature_cols_orig=BASE_COLS,
            engineered_features=ENGINEERED, platt_a=1.04, platt_b=0.02,
            pruned_features=['macd_hist'], zero_features=[],
            pre_excluded_features=['funding_rate_change'], recency_enabled=False,
            recency_halflife=90, cv_folds=5, num_boost_round=1200, early_stopping=80,
    ...
    def test_feature_counts_reconcile(self) -> None:
        report = "\n".join(self._report())
        assert f"Features: {len(FEATURE_COLS)} ({len(BASE_COLS)} base + {len(ENGINEERED)} engineered)" in report
