...
def _lgb_cv(X, y, cols, embargo, n_folds=DEFAULT_N_CV_FOLDS):
    return lgb_walk_forward_cv(X, y, default_lgb_params(), None, n_folds, True,
...
class TestTreeTraversal:
    ...
    def _node(self, **extra) -> dict:
        return {'split_feature': 0, 'threshold': 0.5,
                'left_child': self.LEAF_L, 'right_child': self.LEAF_R, **extra}
...
class TestBrowserModelContract:
    ...
    def test_exported_fields_and_derived_gaps(self) -> None:
        ...
        model = build_lgb_browser_model(
            trees, feature_cols=['a', 'b'], init_score=0.25,
            platt_a=1.5, platt_b=-0.25, platt_on_logits=True,
            accuracy=0.80, auc=0.90, brier=0.15,
            calibration={'ece': 0.0123456, 'mce': 0.2}, cv_auc=0.85, cv_acc=0.75,
        ...
        assert set(model['metrics']) == {
            'accuracy', 'auc', 'brier', 'calibration_ece', 'calibration_mce',
            'cv_auc', 'cv_acc', 'cv_test_acc_gap', 'cv_test_auc_gap',
        }
...
class TestFoldArithmetic:
    ...
    def test_oof_rows_match_the_xgboost_folds_exactly(self, separable_dataset) -> None:
        ...
        *_, xgb_idx = walk_forward_cv(
            X, y, {'max_depth': 3, 'learning_rate': 0.3}, n_folds=DEFAULT_N_CV_FOLDS,
            return_preds=True, feature_cols=cols, embargo=embargo,
            seed=42, **FAST_KW,
        )
    ...
    def test_boosting_budget_matches_the_xgboost_stage(self) -> None:
        ...
        assert (LGB_BOOST_ROUND, LGB_EARLY_STOPPING) == (DEFAULT_NUM_BOOST_ROUND,
...
class TestTrainAndCalibrate:
    ...
    def trained(self, separable_dataset):
        X, y, cols = separable_dataset
        split = 450
        model = train_final_lgb(X[:split], y[:split], None, X[split:], y[split:],
                                default_lgb_params(), feature_cols=cols, **FAST_KW)
    ...
    def test_calibration_is_skipped_without_enough_oof_margins(self, trained) -> None:
        model, X, y, _, split = trained
        cal = fit_lgb_platt(model, X[split:], y[split:],
                            np.zeros(10), np.zeros(10, dtype=int), raw_auc=0.9)
