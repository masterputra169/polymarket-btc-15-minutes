...
ENSEMBLE_KEYS = ['ensemble_weights', 'ensemble_metrics',
                 'lgb_platt_a', 'lgb_platt_b', 'lgb_platt_on_logits']

ENSEMBLE_METRIC_KEYS = ['accuracy', 'auc', 'logloss', 'brier', 'calibration_ece',
                        'calibration_mce', 'weight_source', 'test_samples',
                        'holdout_samples', 'strict_holdout']
...
class _MarginBooster:
    ...
    def predict(self, data: Any, output_margin: bool = False,
...
def splits() -> dict[str, Any]:
    ...
    return {
        'X_train': X[:480], 'y_train': y[:480],
        'X_holdout': X[480:540], 'y_holdout': y[480:540],
        'X_test': X[540:], 'y_test': y[540:],
...
def xgb_oof(splits: dict[str, Any]) -> tuple:
    ...
    return walk_forward_cv(
        ...
        {'max_depth': 3, 'learning_rate': 0.3}, None, N_FOLDS, True, None, False,
        feature_cols=splits['feature_cols'], seed=42, embargo=EMBARGO,
        num_boost_round=40, early_stopping=10,
    )


def _run(splits: dict[str, Any], xgb_oof: tuple, out_dir, log,
         *, xgb_model: Any = None, norm: dict[str, Any] | None = None) -> LightGbmStageResult:
    ...
    return run_lightgbm_stage(
        X_train=splits['X_train'], y_train=splits['y_train'], w_train=None,
        X_final_train=splits['X_train'], y_final_train=splits['y_train'],
        w_train_final=None,
        X_test=splits['X_test'], y_test=splits['y_test'],
        X_holdout=splits['X_holdout'], y_holdout=splits['y_holdout'],
        feature_cols=splits['feature_cols'],
        seed=42, embargo=EMBARGO, n_folds=N_FOLDS,
        use_optuna=False,
        xgb_model=xgb_model or _ExplodingBooster(), xgb_dholdout=None,
        xgb_oof_margins=oof_margins, xgb_oof_labels=oof_labels, xgb_oof_idx=oof_idx,
        xgb_platt_a=1.0, xgb_platt_b=0.0,
        xgb_test_probs=np.full(len(splits['y_test']), 0.5),
        xgb_accuracy=0.6, xgb_auc=0.7,
        norm=dict(BASE_NORM) if norm is None else norm,
        output_dir=str(out_dir),
        strict_holdout=True,
        num_boost_round=60, early_stopping=15,
        log=log,
    )
...
def test_norm_export_appends_ensemble_block_in_order(splits, xgb_oof, tmp_path, capsys):
    ...
    assert written['ensemble_weights'] == {'xgb': result.weight_xgb,
                                           'lgb': result.weight_lgb}
...
def test_result_matches_exported_lightgbm_model(splits, xgb_oof, tmp_path):
    ...
    assert exported['ensemble_weights'] == {'xgb': result.weight_xgb,
                                            'lgb': result.weight_lgb}
...
def test_resolve_params_without_optuna_returns_defaults():
    ...
    params = _resolve_lgb_params(
        np.zeros((4, 2), dtype=np.float32), np.zeros(4, dtype=np.int32), None,
        feature_cols=['a', 'b'], seed=42, embargo=0, n_folds=2,
        use_optuna=False, log=lines.append,
    )
    assert params == default_lgb_params()
    assert lines == ["   Using default LightGBM params (no Optuna)"]


def _sweep(*, xgb_margins, lgb_margins, xgb_idx, lgb_idx, X_holdout,
           xgb_model, lgb_model, log, n_test=40):
    labels = np.tile([0, 1], max(len(xgb_margins), 1) // 2 + 1)[:len(xgb_margins)]
    return _select_ensemble_weight(
        xgb_oof_margins=xgb_margins, lgb_oof_margins=lgb_margins,
        xgb_oof_labels=labels, xgb_oof_idx=xgb_idx, lgb_oof_idx=lgb_idx,
        xgb_platt_a=1.0, xgb_platt_b=0.0, lgb_platt_a=1.0, lgb_platt_b=0.0,
        xgb_model=xgb_model, xgb_dholdout=None, lgb_model=lgb_model,
        X_holdout=X_holdout, y_holdout=np.tile([0, 1], 30),
...
def test_sweep_realigns_and_warns_when_oof_rows_differ():
    ...
    weight, source = _sweep(
        xgb_margins=rng.normal(size=300), lgb_margins=rng.normal(size=300),
        xgb_idx=xgb_idx, lgb_idx=lgb_idx, X_holdout=None,
        xgb_model=_ExplodingBooster(), lgb_model=_ExplodingBooster(), log=lines.append,
    )
...
def test_sweep_falls_back_to_holdout_when_oof_is_empty():
    ...
    empty = np.array([])
    weight, source = _sweep(
        xgb_margins=empty, lgb_margins=empty,
        xgb_idx=empty, lgb_idx=empty,
...
def test_sweep_falls_back_to_test_without_holdout():
    ...
    weight, source = _sweep(
        xgb_margins=empty, lgb_margins=empty,
        xgb_idx=empty, lgb_idx=empty, X_holdout=None,
