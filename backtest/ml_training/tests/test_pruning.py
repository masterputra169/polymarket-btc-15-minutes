...
class _StubBooster:
    ...
    def __init__(self, gains: dict[str, float], predictions: np.ndarray | None = None) -> None:
        self._gains = gains
...
def _cv_returning(fold_importances: list[dict[str, float]]):
    ...
    def cv_fn(X, y, cfg, w, *, return_importances: bool = False, feat_weights=None, **kwargs):
...
def _call(
    ...
    *,
    ...
    return evaluate_pruning(
        ...
        feat_weights=None,
        pre_exclude_fw=(
            np.ones(len(cols), dtype=np.float32) if pre_exclude_fw is None else pre_exclude_fw
        ),
...
class TestFoldStabilityFilter:
    ...
    def _run(self, log=None):
        ...
        return _call(
            _StubBooster(self.GAINS),
            self.COLS,
            _cv_returning(self.FOLDS),
            X=X,
            y=y,
            dtest=dtest,
            y_test=y,
    ...
    def test_rescue_is_reported_by_name(self) -> None:
        lines: list[str] = []
        self._run(log=lines.append)
        assert any("Rescued by fold stability" in line and "rescued" in line for line in lines)
...
class TestKeepOrRejectRetrain:
    ...
    def test_holdout_outranks_test_for_the_keep_decision(self) -> None:
        ...
        result = evaluate_pruning(
            stub,
            feature_cols=cols,
            cv_fn=_cv_returning([gains, gains]),
            X_train=X_tr[:300],
            y_train=y_tr[:300],
            w_train=None,
            best_cfg={"max_depth": 3},
            ...
            has_pre_excluded=False,
            final_params=RETRAIN_PARAMS,
            X_final_train=X_tr[:300],
            y_final_train=y_tr[:300],
            w_train_final=None,
            num_boost_round=25,
            early_stopping=10,
            dtest=dtest,
