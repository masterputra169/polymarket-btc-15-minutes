...
parser.add_argument(
    "--fresh",
    default="polymarket_history_fresh.csv",
    help="Freshly fetched market history CSV from fetchPolymarketHistory.mts",
)
parser.add_argument(
    "--lookup", default="polymarket_lookup.json", help="Existing polymarket_lookup.json to update"
)
parser.add_argument(
    "--days", type=int, default=7, help="Only process markets newer than N days (default: 7)"
)
parser.add_argument(
    "--no-prices",
    action="store_true",
...
parser.add_argument(
    "--clob-token-file",
    default="polymarket_btc15m_data/raw_btc15m_markets.json",
...
def fetch_clob_prices(token_id, start_ts_sec, end_ts_sec):
    ...
    if not data or "history" not in data:
        return []
    return [
        [int(p["t"]) - start_ts_sec, round(float(p["p"]), 6)]
...
def main():
    ...
    print(
        f"Days cutoff: last {args.days} days (>{datetime.fromtimestamp(CUTOFF_MS/1000, tz=UTC).strftime('%Y-%m-%d')})"
    )
    ...
    for i, m in enumerate(new_markets):
        ...
        if (i + 1) % 10 == 0:
            print(
                f"   {i+1}/{len(new_markets)} — {added} added, {skipped_prices} without tick prices"
            )
            sys.stdout.flush()
