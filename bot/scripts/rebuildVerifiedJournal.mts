...
type MarketInfo = {
  market: ClobMarketResponse | null;
  outcome: string | null;
  resolved: boolean;
};
...
const { BOT_CONFIG } = await import('../src/config.ts');
const { initClobClient, getTradeHistory, getProxyAddress, isClientReady } = await import('../src/trading/clobClient.ts');
...
const OUTPUT_FILE = typeof ARGS.output === 'string'
  ? ARGS.output
  : resolve(BOT_CONFIG.dataDir, 'verified_journal.v2.jsonl');
const DRY_RUN = 'dry-run' in ARGS;
const LIMIT = parseInt(String(ARGS.limit ?? '0'), 10);
...
async function httpGet<T = unknown>(url): Promise<T | null> {
...
async function fetchMarketInfo(conditionId): Promise<MarketInfo> {
  const cached = marketCache.get(conditionId);
  if (cached) return cached;
  const url = `${CLOB_BASE}/markets/${conditionId}`;
  const data = await httpGet<ClobMarketResponse>(url);
  if (!data) {
