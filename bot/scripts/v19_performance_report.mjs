...
const SINCE_MS = args['since-ms'] ? parseInt(args['since-ms'], 10) : detectDeployTime();
...
// ── Aggregate ──
const allStats = { w: 0, l: 0, p: 0, pnl: 0 };
const byEntry  = {};   // type -> {w,l,p,pnl}
const byPhase  = {};   // phase -> {...}
const byBand   = {};   // confBand -> {...}
const byEntryPhase = {}; // type:phase -> {...}

function bucket(map, key) { return map[key] ?? (map[key] = { w: 0, l: 0, p: 0, pnl: 0 }); }
...
const shadowWR = { EARLY: 0.69, MID: 0.82, LATE: 0.90, VERY_LATE: 0.99 };
