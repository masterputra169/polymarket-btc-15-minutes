...
/**
 * Backward-compatible live BTC/USD fetcher for setup and diagnostic scripts.
 * The trading loop uses initDataStreams/getLatestStreamPrice directly.
 */
export async function fetchBTCPrice() {
  ...
  while (Date.now() - startedAt < 10_000) {
    const latest = getLatestStreamPrice();
    if (latest) return { ...latest, source: 'data_streams' };
    await new Promise<void>((resolve) => setTimeout(resolve, 500));
  }

  const latest = getLatestStreamPrice();
  return latest ? { ...latest, source: 'data_streams' } : null;
}

export const getBTCPrice = fetchBTCPrice;
