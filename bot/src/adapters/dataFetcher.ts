...
type BotOrderBookSummary = ReturnType<typeof summarizeOrderBook> & {
  error?: boolean;
};
...
export async function fetchPolymarketSnapshot() {
  try {
    ...
    let upBuy = null;
    let downBuy = null;
    let upBookSummary: BotOrderBookSummary = { bestBid: null, bestAsk: null, spread: null, bidLiquidity: null, askLiquidity: null, error: false };
    let downBookSummary: BotOrderBookSummary = { bestBid: null, bestAsk: null, spread: null, bidLiquidity: null, askLiquidity: null, error: false };
...
export async function fetchChainlinkBtcUsd() {
  ...
  try {
    ...
    if (cachedDecimals === null || (now - cachedDecimalsTs >= DECIMALS_TTL)) {
      ...
      const decJson = await decRes.json() as RpcJson;
    ...
    const roundJson = await roundRes.json() as RpcJson;
...
export async function fetchChainlinkAtTimestamp(targetSec) {
  ...
  try {
    ...
    const latHex = ((await latRes.json()) as RpcJson).result?.slice(2);
    ...
    const roundFetches = offsets.map(async (off) => {
      ...
      const hex = ((await r.json()) as RpcJson).result?.slice(2);
