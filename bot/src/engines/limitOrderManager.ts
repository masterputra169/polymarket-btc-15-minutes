...
import { getSpreadTrend } from './executionTiming.ts';
...
type MonitorLimitOrderResult = {
  action: string;
  detail: string;
  fillData?: LimitFillData;
};

type CancelLimitOrderResult = {
  cancelled: boolean;
  filledInstead: boolean;
  fillData?: LimitFillData;
};
...
let state = {
  ...
  _lastRejectLogAt: 0,     // Throttle repeated reject logs
};
...
export function evaluateLimitEntry({
  ...
  // v19 verdict (2026-05-15): LIMIT DOWN historically 38% WR (bottom-catch trap).
  // disableDownSide is the hard kill-switch (LIMIT_DISABLE_DOWN). Default OFF —
  // the smart directional guard below protects DOWN instead of blanket-disable.
  if (cfg.disableDownSide && side === 'DOWN') {
    return no('limit_down_disabled');
  }

  // BTC-PTB consensus gate (ASYMMETRIC — audit 2026-05-15):
  //   UP:   loose buffer (0.3%) — BTC oscillates, don't over-block UP (71% WR side)
  //   DOWN: TIGHT — only allow when BTC at/near/below PTB (downtrend confirmed).
  //         Root-cause fix for bottom-catch trap: prev 0.3% buffer let DOWN fill
  //         at BTC +0.08% above PTB ($69@$80k) → fails when BTC bounces. Now DOWN
  //         requires distPct <= downMaxAbovePtbPct (default 0.05%, env-tunable).
  const PTB_BUFFER_PCT = 0.003;
  const DOWN_MAX_ABOVE = cfg.downMaxAbovePtbPct ?? 0.0005;
  if (btcPrice != null && priceToBeat != null && Number.isFinite(btcPrice) && Number.isFinite(priceToBeat)) {
    ...
    if (side === 'DOWN' && distPct > DOWN_MAX_ABOVE) {
      // DOWN bet needs BTC at/below PTB — block bottom-catch (BTC well above PTB)
      return no(`down_no_confirm_btc_+${(distPct * 100).toFixed(3)}%>${(DOWN_MAX_ABOVE * 100).toFixed(2)}%`);
    }
  }
...
}, deps): Promise<MonitorLimitOrderResult> {
  const cfg = BOT_CONFIG.limitOrder;
 ...
 */
export async function cancelLimitOrder(reason, deps): Promise<CancelLimitOrderResult> {
