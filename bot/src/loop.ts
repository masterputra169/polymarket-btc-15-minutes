...
import { CONFIG, BOT_CONFIG } from './config.ts';
...
import {
  ...
  onMarketSwitch,
  saveFeedbackToDisk,
} from './adapters/feedbackStore.ts';

// Signal performance (JSON file persistence)
import { saveSignalPerfToDisk } from './adapters/signalPerfStore.ts';

// Shared pure JS modules
import { computeBetSizing } from '../../src/engines/asymmetricBet.ts';
import { decide } from '../../src/engines/edge.ts';
import {
...
import {
  ...
  clearPendingOrders,
} from './trading/fillTracker.ts';

// Trading
import { placeBuyOrder, placeLimitBuyOrder, getOrderById, getOrderStatus, cancelOrder, isClientReady, getUsdcBalance, getOpenOrders, cancelAllOrders, getTradeHistory, updateConditionalApproval, getConditionalTokenBalance } from './trading/clobClient.ts';
import {
  recordTrade,
  settleTrade,
...
} from './trading/positionTracker.ts';
import { closePosition } from './trading/positionManager.ts';
import { evaluateCutLoss, resetCutLossState, recordSellAttempt, resetCutConfirm, getCutLossStatus } from './trading/cutLoss.ts';
import { evaluateTakeProfit, resetTakeProfitState } from './trading/takeProfit.ts';
import { onCutLoss as recoveryOnCutLoss, tick as recoveryTick, reset as resetRecovery, getRecoveryStatus, isRecoveryActive } from './trading/recoveryBuy.ts';
import { captureEntrySnapshot, writeJournalEntry, clearEntrySnapshot, getEntrySnapshot, getRecentJournal } from './trading/tradeJournal.ts';
import { getJournalAnalytics } from './trading/journalAnalytics.ts';
import {
  ...
  recordFillEvent,
} from './engines/limitOrderManager.ts';
import { routeOrder } from './engines/orderRouter.ts';

// Safety
import { shouldHalt, shouldEmergencyCut, validateTrade, validatePrice, recordTradeTimestamp, exportTradeTimestamps, importTradeTimestamps } from './safety/guards.ts';
import {
  applyTradeFilters,
  recordLoss,
...
} from './safety/tradeFilters.ts';

// Daily profit target (WIB timezone)
import { checkProfitTarget, getProfitTargetStatus, isTargetReached, initDayBaseline } from './safety/dailyProfitTarget.ts';

// Orderbook flow tracking
import { checkFlowAlignment, resetFlow } from './engines/orderbookFlow.ts';

// Smart money flow tracking (time-windowed CLOB flow analysis)
import { updateSmartFlow, getSmartFlowSignal, getEntryTimingScore, resetSmartFlow } from './engines/smartMoneyTracker.ts';

// MetEngine smart money API gates (F1: consensus, F2: insider score, F3: conviction wallet)
import { initMetEngine, querySmartMoney, clearMetEngineCache, getMetEngineStats } from './engines/metEngineClient.ts';

// Sentiment signal (free APIs: Fear & Greed, BTC Dominance)
import { fetchSentiment, getSentiment, getSentimentStats, checkExtremeSentiment } from './engines/sentimentSignal.ts';
import { setContextSnapshot as setLLMRegimeSnapshot } from './ai/regimeClassifier.ts';

// Execution timing optimizer
import { evaluateExecutionTiming, getExecutionTimingStatus, resetExecutionTiming, recordSpreadTick, recordVolumeTick } from './engines/executionTiming.ts';

// AI analysis (dashboard display)
import { getLastAnalysis } from './ai/postTradeAnalyst.ts';
...
} from './engines/signalStability.ts';
import {
  getRefreshNeeds,
  updateKlines5m,
  updateChainlink,
...
let currentMarketEndMs: number | null = null;
...
let ptbScheduleTimer: ReturnType<typeof setTimeout> | null = null;
let ptbScheduledPrice: ScheduledPrice | null = null;
...
let settlementAbort: AbortController | null = null; // AbortController: aborts oracle retries when market switches → instant price_fallback
...
function resetMarketCache() {
  ...
  priceToBeat = { slug: null, value: null, source: null, updatedAt: 0 };
...
export async function pollOnce() {
  ...
  try {
    ...
    const fetches: Promise<any>[] = [fetchKlines({ interval: '1m', limit: 240 })];
    const fetchMap: FetchMap = { klines1m: 0 };
    ...
    // ── 7. Decision ──
    const rec: ReturnType<typeof decide> & { strength?: string; edge?: number } = decide({
