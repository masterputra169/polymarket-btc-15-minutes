...
type TrackerState = {
  ...
  losses: number;
  trades: any[];
  lastSettledSlug: string | null;
  lastSettledTs: number;
  lastSettlementMs: number;
  cutLossCount: number;
  marketTradeCounts: Record<string, number>;
 ...
 */
export function recordTrade({ side, tokenId, conditionId, price, size, marketSlug, orderId, actualCost }: RecordTradeInput) {
 ...
 */
export function recordArbTrade({ upCost, downCost, shares, marketSlug, orderId, conditionId }: RecordArbTradeInput) {
 ...
 */
export function _resetForTest(overrides: Partial<TrackerState> = {}) {
  state = {
