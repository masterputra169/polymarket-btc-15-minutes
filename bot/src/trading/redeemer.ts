...
type DataPositionsResponse = any[] | {
  data?: any[];
  results?: any[];
};

type ClobMarketResponse = {
  closed?: boolean | string | number;
  tokens?: any[];
  question?: string;
};

type GammaMarketsResponse = any[] | {
  markets?: any[];
};
...
async function findRedeemablePositions() {
  ...
  try {
    ...
    while (page < MAX_PAGES) {
      ...
      const raw = await res.json() as DataPositionsResponse;
  ...
  for (const pos of candidates) {
    ...
    try {
      ...
      if (res.ok) {
        const market = await res.json() as ClobMarketResponse;
    ...
    try {
      ...
      if (gammaRes.ok) {
        const gammaData = await gammaRes.json() as GammaMarketsResponse;
...
async function redeemCycle() {
  ...
  if (redeemed > 0) {
    try {
      const { queueUsdcSyncAfterRedeem } = await import('../engines/usdcSync.ts');
