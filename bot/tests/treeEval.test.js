...
beforeAll(async () => {
  const S = await import('../../src/engines/ml/state.js');
...
describe('indexTree', () => {
  it('handles single leaf node', async () => {
    const { indexTree } = await import('../../src/engines/ml/treeEval.js');
  ...
  it('indexes tree with children', async () => {
    ...
    expect(tree.get(0)?.threshold).toBe(0.5);
    expect(tree.get(1)?.leaf).toBe(0.7);
    expect(tree.get(2)?.leaf).toBe(-0.3);
  });
});

describe('evaluateTreeFast', () => {
  it('routes correctly on threshold split (yes branch)', async () => {
    const { indexTree, evaluateTreeFast } = await import('../../src/engines/ml/treeEval.js');
    const tree = indexTree({
      nodeid: 0, split: 'rsi', split_condition: 0.5, yes: 1, no: 2,
      children: [
        { nodeid: 1, leaf: 1.0 },
