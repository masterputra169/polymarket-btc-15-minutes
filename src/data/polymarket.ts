...
export interface FetchClobPriceOptions {
  tokenId: string;
  side: string;
}
...
export type PolymarketSnapshot =
  | {
      ok: true;
      market: GammaMarket;
      tokens: { upTokenId: string | null; downTokenId: string | null };
...
export async function fetchLiveEventsBySeriesId({
  seriesId,
  limit = 20,
}: FetchLiveEventsOptions): Promise<GammaEvent[]> {
...
export function flattenEventMarkets(events: unknown): GammaMarket[] {
  const out: GammaMarket[] = [];
  for (const e of Array.isArray(events) ? events : []) {
    const event = e as GammaEvent;
...
function safeTimeMs(x: unknown): number | null {
  if (!x) return null;
  if (!(typeof x === 'string' || typeof x === 'number' || x instanceof Date)) return null;
  const t = new Date(x).getTime();
  return Number.isFinite(t) ? t : null;
}

export function pickLatestLiveMarket(markets: unknown, nowMs = Date.now()): GammaMarket | null {
...
export async function fetchClobPrice({ tokenId, side }: FetchClobPriceOptions): Promise<number | null> {
...
export async function fetchOrderBook({ tokenId }: FetchOrderBookOptions): Promise<unknown> {
...
export function summarizeOrderBook(book: any, depthLevels = 5): OrderBookSummary {
  const bids: OrderBookLevel[] = Array.isArray(book?.bids) ? book.bids : [];
  const asks: OrderBookLevel[] = Array.isArray(book?.asks) ? book.asks : [];
...
export async function fetchPolymarketSnapshot(
  { skipClob = false }: { skipClob?: boolean } = {}
  ...
  try {
    ...
    const safeParse = (v: string): unknown[] => { try { const parsed = JSON.parse(v); return Array.isArray(parsed) ? parsed : []; } catch { return []; } };
    ...
    let upBookSummary: OrderBookSummary = { bestBid: null, bestAsk: null, spread: null, bidLiquidity: null, askLiquidity: null };
