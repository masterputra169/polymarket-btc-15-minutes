...
export function initLoggerDB(): Promise<boolean> {
...
export function logSnapshot(row: Record<string, any>): void {
  if (!db) return;
  lastLogMs = Date.now();
  try {
    const tx = db.transaction(STORE_NAME, 'readwrite');
...
export async function exportToCSV(): Promise<void> {
  ...
  const rows = await new Promise<TrainingLogRow[]>((resolve) => {
    const tx = db.transaction(STORE_NAME, 'readonly');
    const req = tx.objectStore(STORE_NAME).index('timestamp').getAll();
    req.onsuccess = () => resolve(req.result as TrainingLogRow[]);
    req.onerror = () => resolve([]);
  });
  ...
  const labeled: Array<TrainingLogRow & { label: number; futurePrice: number }> = [];
