...
import { computeBetSizing } from '../asymmetricBet.ts';
...
describe('regime multiplier', () => {
  it('choppy at full confidence → 0.60 multiplier', () => {
    const result = computeBetSizing(makeParams({
      regimeInfo: { regime: 'choppy', confidence: 1.0 },
    }));
    expect(result.regimeAdj.multiplier).toBeCloseTo(0.60, 2);
  });
  ...
  it('mean_reverting at full confidence → 0.75', () => {
    const result = computeBetSizing(makeParams({
      regimeInfo: { regime: 'mean_reverting', confidence: 1.0 },
    }));
    expect(result.regimeAdj.multiplier).toBeCloseTo(0.75, 2);
  });

  it('low confidence scales multiplier toward 1.0', () => {
    // regime=choppy, base=0.60, confidence=0.2
    // mult = 1.0 + (0.60 - 1.0) * 0.2 = 1.0 - 0.08 = 0.92
    const result = computeBetSizing(makeParams({
      regimeInfo: { regime: 'choppy', confidence: 0.2 },
    }));
    expect(result.regimeAdj.multiplier).toBeCloseTo(0.92, 2);
  });
...
describe('ML multiplier', () => {
  it('high-conf agreement → 1.15', () => {
    const result = computeBetSizing(makeParams({
      side: 'UP',
      ml: { status: 'ready', side: 'UP', confidence: 0.75 },
    }));
    expect(result.mlAdj.multiplier).toBeCloseTo(1.15, 2);
  });

  it('high-conf disagreement → 0.50', () => {
    const result = computeBetSizing(makeParams({
      side: 'UP',
      ml: { status: 'ready', side: 'DOWN', confidence: 0.80 },
    }));
    expect(result.mlAdj.multiplier).toBeCloseTo(0.50, 2);
  });
...
describe('confidence tier', () => {
  ...
  it('MEDIUM → 0.65', () => {
    ...
    expect(result.confidenceAdj.multiplier).toBe(0.65);
  });

  it('LOW → 0.40', () => {
    const result = computeBetSizing(makeParams({ confidence: 'LOW' }));
    expect(result.confidenceAdj.multiplier).toBe(0.40);
  });
