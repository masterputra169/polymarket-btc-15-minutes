...
export interface MlStateUpdates {
  processedTrees?: any[] | null;
  numUsableTrees?: number;
  isLoading?: boolean;
  loadError?: unknown;
  modelMemoryKB?: number;
  modelVersion?: number;
  modelNumFeatures?: number;
  optimalThreshold?: number;
  modelMetrics?: any;
  featureNameToIdx?: Map<string, number> | null;
  plattA?: number;
  plattB?: number;
...
export function setBaseFeatureCount(count: number): void {
  BASE_FEATURES = count;
  MAX_FEATURES = count + ENGINEERED_FEATURES;
}

// Module state
export let processedTrees: any[] | null = null;
export let numUsableTrees = 0;
export let isLoading = false;
export let loadError: unknown = null;
...
export let modelMetrics: any = null;
export let featureNameToIdx: Map<string, number> | null = null;
...
export function setState(updates: MlStateUpdates): void {
