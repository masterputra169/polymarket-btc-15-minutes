...
export type MultiTfSignal = 'UP' | 'DOWN' | 'NEUTRAL';

export interface MultiTfParams {
  delta1m?: number | null;
  delta3m?: number | null;
  delta5m?: number | null;
  ha1mColor?: string | null;
  ha5mColor?: string | null;
  rsi1m?: number | null;
  rsi5m?: number | null;
}
...
export function computeMultiTfConfirmation({
  ...
  rsi1m = null,
  rsi5m = null,
}: MultiTfParams): MultiTfConfirmation {
  let upVotes = 0;
  let downVotes = 0;
  let totalVoters = 0;  // H5 multi-TF fix: track how many data points actually voted
  let tf1mSignal: MultiTfSignal = 'NEUTRAL';
  let tf5mSignal: MultiTfSignal = 'NEUTRAL';
  ...
  const signal: MultiTfSignal = upVotes > downVotes ? 'UP' : upVotes < downVotes ? 'DOWN' : 'NEUTRAL';
