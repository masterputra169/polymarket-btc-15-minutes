...
import { fetchPolymarketSnapshot } from '../data/polymarket.ts';
import { fetchChainlinkBtcUsd } from '../data/chainlinkRpc.ts';
import { fetchFundingRate } from '../indicators/fundingrate.ts';
import { scoreDirection, applyTimeAwareness } from '../engines/probability.ts';
import { computeEdge, decide } from '../engines/edge.ts';
import { computeBetSizing } from '../engines/asymmetricBet.ts';
import { analyzeOrderbook } from '../engines/orderbook.ts';
import { getAccuracyStats, getDetailedStats, recordPrediction, autoSettle, onMarketSwitch, getSignalModifiers } from '../engines/feedback.ts';
import { loadMLModel, getMLPrediction, getMLStatus, getTrainedSignalModifiers, getCalibratedPhaseThresholds } from '../engines/Mlpredictor.ts';
import {
...
import { computeAllIndicators } from './computeIndicators.ts';
 ...
 */
export function useMarketData({ clobWs }: any = {}) {
      ...
      try {
        ...
        if (rec.action === 'ENTER' && rec.side && marketSlug) {
          recordPrediction({
            ...
            mlConfidence: mlResult.available ? mlResult.mlConfidence : null,
            mlSide: mlResult.available ? mlResult.mlSide : rec.side,
            breakdown: scored.breakdown,
          });
        }
