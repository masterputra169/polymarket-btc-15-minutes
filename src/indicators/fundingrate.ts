...
let cachedFunding: FundingRateResult | null = null;
let lastFetchMs = 0;
let workingHost: string | null = null; // Remember which host works
 ...
 */
async function fetchFapiWithFallback(path: string): Promise<any | null> {
 ...
 */
async function fetchBybitFundingRate(): Promise<FundingRateResult | null> {
  try {
    ...
    let sentiment: FundingSentiment = 'NEUTRAL';
 ...
 */
async function doFetch(): Promise<FundingRateResult | null> {
  try {
    ...
    console.warn('[FundingRate] All hosts failed:', err instanceof Error ? err.message : String(err));
    return cachedFunding;
  }
}
 ...
 * @returns {Promise<{ rate: any, ratePct: any, extreme: any, sentiment: any, nextFundingTime: any, fetchedAt: any, source: any } | null>}
 */
export async function fetchFundingRate(): Promise<FundingRateResult | null> {
  const now = Date.now();
 ...
 */
export function getCachedFundingRate(): FundingRateResult | null {
  return cachedFunding;
}
