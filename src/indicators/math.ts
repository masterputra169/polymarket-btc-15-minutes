 ...
 */
export function sma(arr: number[] | null | undefined, period: number): number | null {
 ...
 */
export function slopeLast(arr: number[] | null | undefined, n = 3): number | null {
 ...
 */
export function emaFull(data: number[] | null | undefined, period: number): number[] {
...
export function emaSeries(
  ...
  const k = 2 / (period + 1);
  const result: Array<number | null> = new Array(data.length);
