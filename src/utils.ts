...
export function formatProbPct(p: unknown, digits = 0): string {
...
export function fmtTimeLeft(mins: number): string {
...
export function fmtEtTime(now = new Date()): string {
  try {
...
export type MlSessionName = 'EU/US Overlap' | 'Asia' | 'Europe' | 'US' | 'Off-hours';
...
export function getBtcSession(now = new Date(), hours: SessionHours = DEFAULT_SESSION_HOURS): BtcSession {
...
export function formatSignedDelta(delta: number | null | undefined, base: number | null | undefined): string {
...
export type DirectionNarrative = 'LONG' | 'SHORT' | 'NEUTRAL';

export function narrativeFromSign(x: unknown): DirectionNarrative {
  if (x === null || x === undefined || !Number.isFinite(Number(x)) || Number(x) === 0) return 'NEUTRAL';
  return Number(x) > 0 ? 'LONG' : 'SHORT';
}

export function narrativeFromRsi(rsi: unknown): DirectionNarrative {
...
export function narrativeFromSlope(slope: unknown): DirectionNarrative {
  if (slope === null || slope === undefined || !Number.isFinite(Number(slope)) || Number(slope) === 0) return 'NEUTRAL';
  return Number(slope) > 0 ? 'LONG' : 'SHORT';
}

export function getSessionName(now = new Date(), hours: SessionHours = DEFAULT_SESSION_HOURS): MlSessionName {
...
export function parsePriceToBeat(text: unknown): number | null {
...
interface PriceToBeatCandle {
  openTime: number;
  open: number;
}

export function extractPriceToBeat(
  market: PriceToBeatMarket | null | undefined,
  klines?: PriceToBeatCandle[] | null
): number | null {
  ...
  if (
    (typeof startTime === 'string' || typeof startTime === 'number' || startTime instanceof Date) &&
    ...
    if (Number.isFinite(startMs)) {
      ...
      let best: PriceToBeatCandle | null = null;
